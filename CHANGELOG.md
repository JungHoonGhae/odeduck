...
- 활용신청 확인 후 폼·대상 데이터·상세기능·활용목적·분류·동의·제출 경로가 바뀌면 제출을 차단합니다.
  확인 상태는 브라우저 안에 보관하고, 재검증과 1회 제출을 같은 실행에서 처리합니다.
- 제출 후 고정 4초 대기를 제거하고 포털 완료·검증 오류 응답을 최대 15초 기다립니다.
  응답이 없으면 신청내역을 확인하고, 결과가 불확실하면 자동 재제출하지 않고 확인 필요 오류를 반환합니다.

### Validation and limits

- 로컬 Chrome의 기존 REST·ODCloud 폼, 확인 후 필드·DOM·URL 변경, 1회 제출, 취소와 응답 대기를 검증했습니다.
- 실제 정부 계정에 새 활용신청을 제출하는 검증은 수행하지 않았습니다. 포털의 장기 가용성이나 속도 개선율을 보장하지 않습니다.
...
- CLI는 `odeduck goal`, MCP tool은 `goal`을 사용합니다. 데이터셋을 지정하지 않은 발견·조사·보고서·분석
  요청은 목표 실행으로 시작하며, 검색은 필요한 자료를 찾는 단계입니다. CLI의 `solve`는 기존 스크립트용으로 남습니다.
- `--review-with codex|claude|gemini`로 고정된 계획·검토 수신자와 선택 근거 전송을 명시합니다.
  보고·분석·요청 범위 검토에 쓰이던 여러 옵션을 묶었으며, 기존 옵션은 호환용으로 유지합니다.
  CLI `goal`은 이 설정이 빠졌다면 모델 호출 전에 설정 방법을 안내합니다.
- 넓은 발견 목표는 서로 다른 구체적 가설을 세우고 실제 후보를 검사하며 수정하도록 공통 계획 지침을 개선했습니다.
  소재·데이터셋·결합 정답을 지침에 고정하지 않습니다.

### Fixed

- 이전 대화를 참조하는 목표는 시작 시 `context`(CLI `--context`)로 필요한 맥락을 계획과 별도 검토에
  함께 전달할 수 있습니다. 원천 근거·권한·승인과 분리하고 크기·인증정보 검사를 적용합니다.
- 목표 실행 전에 같은 카탈로그와 의미 검색 경로를 점검하여 준비되지 않은 환경에서 계획 호출을 낭비하지 않습니다.
- 의미 검색 환경을 복구한 뒤 MCP의 같은 목표에서 `retry_search`로 진행할 수 있습니다. 원래 검색어·역할·
  목표 계약·실패 이력·만료·예산을 보존하며, 일반 검색으로 조용히 낮추지 않습니다.
- 실행 중인 바이너리·빌드·공통 지침의 해시와 허용 행동을 `runtime`에 표시합니다. 파일이 업데이트돼도
  기존 MCP 프로세스를 새 버전으로 오인하지 않습니다.

### Migration and limits

- MCP의 `advance_goal`은 `goal`로 바뀌었습니다. 서버를 다시 연결하고 새 tool schema를 읽어야 합니다.
- 새 CLI 예: `odeduck goal "사람들이 잘 모르는 흥미로운 사실을 발견하고 출처를 확인해주세요" --review-with codex`
- MCP 검토 설정: `odeduck mcp --review-with codex`. 연결된 host와 지정한 검토 제공자에 선택 근거가 전달됩니다.
- 인증키 보호·근거 크기 제한·별도 검토·완료 기준은 유지합니다. 독립 CLI의 자동 신청, 프로세스 간 재개,
  INTENT.md 전체 목표의 범용 자율 완주가 완료됐다는 의미는 아닙니다.
