...
func loginCmd() *cobra.Command {
	...
	c := &cobra.Command{
		...
		Long: `브라우저 창을 띄워 data.go.kr 에 로그인합니다. 로그인이 끝나면 oddsock 이
...
--keep-browser를 사용하세요. 키체인 비밀번호는 묻지 않습니다.`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			fmt.Fprintln(cmd.ErrOrStderr(), "   이제 `oddsock applications` 로 활용신청 현황을 볼 수 있습니다.")
			return nil
		},
	}
...
func logoutCmd() *cobra.Command {
  ...
  · oddsock 이 만든 Chrome 프로파일(로그인용·headless용)
...
func statusCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if err != nil {
				fmt.Fprintln(cmd.ErrOrStderr(), "세션 없음 — `oddsock login` 을 실행하세요.")
				return nil
			}
