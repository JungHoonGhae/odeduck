...
그래서 오데덕을 만들었습니다. AI 에이전트가 한국 공공데이터로 조사와 분석을 수행하도록 돕는
오픈소스 실행 플랫폼입니다. 목표에 맞는 자료 탐색과 자동 활용신청, 데이터 수집·연결·분석을 하나의
흐름으로 이어갑니다. 위 데모는 그중 자료 탐색과 연결 후보 발견을 보여주는 사례입니다.
목표 실행의 현재 지원 범위와 검증 상태는 [README](../../README.md#목표에서-산출물까지-solve)에 정리했습니다.
