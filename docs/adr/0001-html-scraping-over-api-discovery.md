...
opendatactl originally read data.go.kr by scraping HTML, which looks fragile enough that
...
only and allows at most 1,000 rows per request, so OpenDataCTL does not send a
...
document. OpenDataCTL now falls back to that combined first-party page, fetches
