...
[실험적 분석](#목표에서-결과까지-실행하기--experimental) · [로그인·데이터 취급](#인증과-데이터-취급)
...
![목표를 역할별로 탐색하고 원천을 검사해 조회·연결·계산한다. 필수 출력과 근거가 부족하면 예산 안에서 재탐색하며, 허용된 검토를 통과한 결과 또는 미완료 사유를 반환한다.](assets/odeduck-goal-flow.png)
