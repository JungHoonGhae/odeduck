...
<a id="목표에서-결과까지-실행하기--experimental"></a>

## 목표에서 결과와 근거까지: goal

CLI와 MCP 모두 `goal`을 사용합니다. 사용자가 목표를 말하면 필요한 자료와 연결 자료를 발견하고,
검사·취득·계산·검토 결과에 따라 대안을 찾습니다. 검색 목록이나 한 행의 생성만으로 완료하지 않습니다.
현재 지원 범위는 설치된 `odeduck goal --help` 또는 MCP의 `odeduck://guide`에서 확인합니다.
...
`--review-with`는 원천 보고, 지원하는 관계·계산, 원천에 근거한 요청 범위의 검토를 함께 허용합니다.
선택 근거는 최대 8개/64 KiB이며 검토 입력은 최대 96 KiB입니다. 별도 검토는 최대 세 번입니다.
인증키는 전송하지 않으며, 공간·인과·사업 판단이나 현장 검증을 자동 승인하지 않습니다.
MCP tool의 인자로 이 권한을 켤 수 없고, 기존 `call_api`·실행 결과 반환과는 별도 정책입니다.

이전 대화의 예시나 조건을 참조한다면 MCP 시작 요청의 `context`, CLI의 선택형 `--context`에 필요한
맥락만 전달합니다. 최대 8000 UTF-8 bytes이며 인증정보를 넣지 않습니다. 계획과 별도 검토에 같은
맥락이 전달되지만 원천 근거·결과 승인·권한으로 취급하지 않습니다. 시작 후 변경할 수 없습니다.

### 실행과 복구

- 실행 전 카탈로그·의미 검색을 점검합니다. 실제 실행 파일·빌드·지침 해시와 허용 행동은 `runtime`에 표시합니다.
- 의미 검색이 중간에 막히면 오류 원인을 복구한 뒤 MCP의 같은 `goal` 세션에서 `retry_search`를 사용합니다. 검색어·역할·목표·만료·실패 기록·예산은 유지합니다. 동일 검색은 최초 포함 최대 세 번입니다.
- API 접근권한이 없으면 신청 목록으로 로그인과 승인을 확인합니다. 로그인이 필요하면 사람이 처리하며, 허용된 활용신청 후 같은 목표의 `retry_sample`로 이어갑니다. 독립 CLI 실행기는 아직 신청을 직접 제출하지 않습니다.
- 기본 실행 한도는 32단계입니다. 모델 CLI의 이용 비용·호출 한도가 적용됩니다.
- JSON의 `artifact`, `evaluation`, `gaps`에서 결과·근거·미충족 조건을 확인합니다. 미완료는 실패 종료 코드로 반환합니다.

`sample_executed`는 제한된 데이터로 계산이 실행됐다는 뜻입니다. 필수 역할·범위·출력과 별도 검토가
...
세션의 최근 `call_api` profile 영수증과 일치해야 한다. `goal`은 장부에 자동 기록하지 않는다.
...
`catalog discover`와 `goal`은 자연어 목표를 선택한 AI 제공자에게 전달한다. `discover`는
