...
첫 CI에서 Windows 체크아웃이 Markdown을 CRLF로 바꾸면 topic 구분자를 찾지 못하는 결함을
발견했다. v0.21.0 릴리스 작업을 공개 전에 취소하고 tag와 실패 기록을 보존했다. LF/CRLF 정규화와
열 개 topic 전체의 회귀 검사를 추가했다. 수정 배포는 v0.21.1이다.
