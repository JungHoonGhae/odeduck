...
| 프로젝트 | 2026-09-01 GitHub 신호 | 실제 범위 | odeduck과의 관계 |
| --- | ---: | --- | --- |
...
검증됨”까지이며, odeduck의 자동 신청이나 교차 도메인 사업 발견에 대한 반복 사용 수요는 아직
검증되지 않았다.

## 가져올 설계

1. **공개 읽기 계층 + 로컬 권한 계층 분리**: CKAN MCP와 Public Data Lens처럼 검색·비교는 원격에서
   무설치로 제공하고, 로그인·쿠키·키·활용신청·호출만 로컬 odeduck이 담당한다.
...
5. **계획·배치 실행**: mcp-brasil의 planner/batch 아이디어는 가져오되, 500개 도구를 노출하지 않는다.
   odeduck의 작은 `catalog_search → inspect_dataset → call_api` 표면은 유지한다.
