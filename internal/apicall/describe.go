...
type APISpec struct {
	...
	// odeduck does not follow it: the publishers are a long tail (39 distinct hosts
...
// that odeduck can invoke the provider: InvocationState says whether a
...
// Approval reports the two stages the portal grades separately. odeduck applies
	...
	if spec.EndpointOnly {
		...
		if isLinkAPIType(spec.APIType) {
			...
			spec.Note = "API 유형을 REST 또는 LINK로 확인하지 못했습니다 — 페이지 구조나 유형 표기가 바뀐 것일 수 있습니다. " +
				"odeduck doctor로 점검하고, 유형을 확인하기 전에는 엔드포인트를 추측하거나 call_api로 호출하지 마세요."
		}
		if isRESTAPIType(spec.APIType) && spec.GuideDocURL == "" {
			...
			if guideRowFound {
				...
				spec.Note = "이 페이지에서 상세기능·요청변수와 참고문서 항목 자체를 찾지 못했습니다 — " +
					"페이지 구조가 바뀐 것일 수 있습니다 (odeduck doctor 로 확인). 파라미터를 추측하지 마세요."
			}
		}
	}
...
// swaggerDoc is the slice of Swagger 2.0 odeduck reads. Parameters sit at the
