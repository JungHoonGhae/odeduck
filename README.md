...
우리 모두 양말 한 짝을 잃어버린 경험이 있다. 보통은 서랍 안을 다시 뒤진다. 잘 찾는 사람은 세탁기
뒤, 소파 밑, 어제 입은 바지까지 본다. 물건이 사라지는 데에는 생활의 맥락이 있기 때문이다.

데이터를 연결하는 일도 똑같다. 인구에 관한 질문의 나머지 한 짝이 상권이나 교통 데이터에 있을 수
있다. 답은 한 분야 안에 가지런히 모여 있지 않다.

**오데덕은 공공데이터에서 그런 사람이다.** 질문을 여러 분야로 나누고, 서로 상관없어 보이는 데이터
사이에서 함께 확인할 짝을 찾아온다. 찾은 연결은 정답처럼 꾸미지 않고, 실제 명세와 컬럼으로 어디까지
확인했는지 메모해 둔다.

이 생각의 출발점은 오데덕의 독창적인 철학이 아니다. 스티브 잡스는
[2005년 스탠퍼드 졸업 연설](https://news.stanford.edu/stories/2005/06/youve-got-find-love-jobs-says)에서
리드대학에서 우연히 배운 캘리그래피가 10년 뒤 매킨토시의 타이포그래피로 이어진 경험을
`connecting the dots`라고 설명했다. 당시에는 상관없어 보이던 점도 나중에는 하나의 맥락으로 이어질
수 있다는 이야기다. 오데덕은 이 관점을 공공데이터 탐색에 빌려 온다. 다만 연결을 결론으로 단정하지
않고, 실제 키·명세·컬럼으로 다시 확인할 후보로 보여준다.
...
오데덕의 차이는 검색이나 호출을 처음 만들었다는 데 있지 않다. **다른 분야에 흩어진 데이터를 한
...
오데덕은 약 9.6만 개의 API·파일·외부 링크에서 각 질문에 맞는 후보와 함께 볼 짝을 찾는다. 에이전트는
고른 후보의 실제 명세와 파일 컬럼을 검사한다. REST라면 필요할 때 활용신청을 내고 인증키를 보여주지
않은 채 호출하며, FILE은 내려받아 구조를 관찰한다. 지원하지 않는 외부 제공기관은 공식 경로를
알려주고 멈춘다.

질문은 가게 하나였다. 답의 조각은 인구·상권·사업체처럼 서로 다른 분야와 기관에 있었다. 오데덕은
혼자서는 반쪽인 데이터를 나란히 놓고, 어디까지 연결해 확인할 수 있는지 보여준다.

오데덕이 성공할 가게를 대신 골라주지는 않는다. 대신 감으로 끝나던 사업 아이디어를 어떤 데이터로
...
때만 `odeduck login`으로 data.go.kr에 한 번 로그인한다.
...
검색→상세→호출 MCP 자체는 이미 있다. 오데덕은 그걸 처음 만들었다고 말하지 않는다. 차이는 그
...
열거나 클라이언트를 재시작하고 `/mcp` 또는 `codex mcp list`에서 `odeduck`이 enabled인지 확인한다.
odeduck은 초기화할 때 도구 선택 지침도 함께 제공한다. 사용자가 “최대한 정확하게”, “시맨틱”,
...
- 활용신청 자동화는 포털 HTML에 의존한다. 포털이 바뀌면 깨질 수 있고, `odeduck doctor`와 canary가 이를 감시한다.
...
입력한 자연어 목표를 stdin으로 전달한다. odeduck은 그 로그인 토큰을 읽거나 저장하지 않지만, 목표는
