...
**알고 싶은 건 있는데, 무슨 검색어를 넣어야 할지부터 막힙니다.**
API를 하나씩 찾아 설명을 읽고, 필요한 서비스마다 신청합니다.
승인을 확인한 뒤에는 인증키와 호출 설정을 챙겨야 합니다.
다른 분야에 더 중요한 단서가 있어도, 그 자료가 있는지 모르면 찾아보기 어렵습니다.

오데덕은 **질문에서 출발해, 미처 떠올리지 못한 데이터까지 연결 후보로 찾습니다.**
Codex·Claude·Gemini·Cursor에 연결하면 필요한 자료의 내용 확인과 활용신청·조회도 이어갈 수 있습니다.

[연결 예시](#한-질문에서-예상-밖의-연결까지) · [빠른 시작](#빠른-시작) · [신청·조회](#활용신청과-데이터-조회) · [전체 구조](#전체-아키텍처)

## 한 질문에서, 예상 밖의 연결까지

![검색어 고민, 서비스별 신청과 인증키 설정에서 막히던 사용자가 공매 부동산에 대한 질문을 건넵니다. 오데덕은 실거래가뿐 아니라 상권 변화와 토양오염 조사도 연결 후보로 찾고, 필요한 API의 신청·조회와 함께 볼 자료·연결 이유·출처를 잇습니다.](docs/assets/odeduck-unexpected-connections.png)
...
> 부산 공매 부동산을 살펴보고 싶어. 가격 말고 놓치기 쉬운 점도 찾아줘. 필요한 API는 신청·조회해서 함께 볼 자료와 확인할 점을 정리해줘.
...
> 오데덕으로 부산 공매 부동산을 살펴보고 싶어. 가격 말고 놓치기 쉬운 점도 찾아줘.
