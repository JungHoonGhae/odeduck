...
<p align="center"><strong>공공데이터 탐색부터 자동 활용신청·분석까지</strong><br>
  목표에 맞는 자료 탐색과 접근권한 확보, 데이터 수집·연결·분석을 하나의 흐름으로 이어갑니다.
</p>
...
<p align="center">
  ...
  <a href="#목표에서-산출물까지-goal">목표 기반 분석</a> ·
...
오데덕은 **AI 에이전트가 한국 공공데이터로 조사와 분석을 수행하도록 돕는 오픈소스 실행 플랫폼**입니다.
사용자가 하려는 일을 바탕으로 필요한 자료를 찾고, 접근권한을 준비하고, 실제 데이터를 가져와
비교·계산과 근거 확인까지 이어갑니다.

예를 들어 “시장별 양파 가격을 비교해주세요”라는 요청에는 가격 자료를 찾는 일뿐 아니라
API 활용신청, 등급·단위·기간 확인, 데이터 수집과 계산이 필요합니다. 오데덕은 이 작업들을 연결하고,
어떤 자료로 결과를 만들었는지와 아직 부족한 근거를 함께 남깁니다.

| 하고 싶은 일 | 오데덕이 돕는 방법 |
| --- | --- |
| 질문에 필요한 결과 만들기 | 목표에 맞는 자료를 모아 비교·계산하고, 결과의 출처와 부족한 근거를 함께 남깁니다. |
| 서로 다른 자료 연결하기 | 96,000개 이상의 API·파일·외부 제공기관 자료에서 관련 후보를 찾고, 항목·지역·기간 등 함께 사용할 조건을 확인합니다. |
| 데이터 접근과 수집 진행하기 | 필요한 활용신청을 제출하고, 승인 상태를 확인한 뒤 인증키를 자동으로 넣어 API를 호출합니다. |

**첫 검색은 로그인이나 API 키 없이 시작할 수 있습니다.** 활용신청과 인증이 필요한 조회는 포털 로그인 후 사용할 수 있습니다.
목표 기반 분석의 현재 지원 범위는 [아래 설명](#목표에서-산출물까지-goal)에서 확인할 수 있습니다.

## 빠른 시작

Codex·Claude·Gemini·Cursor에 연결해 자연어로 요청하거나, 터미널에서 직접 사용할 수 있습니다.
...
터미널과 MCP 모두 `goal`을 사용합니다. 검색은 목표 수행 중 필요한 자료를 찾는 단계입니다.
...
- **검증 상태:** 목표 실행은 핵심 개발 기능이며 지원 범위를 넓히며 실제 목표 완주를 검증하고 있습니다. 다양한 목표를 자율적으로 완수하는 능력은 검증 중입니다. 필요한 결과 검토가 끝나지 않으면 미완료로 반환합니다.
- **사용 준비:** MCP에서는 연결된 AI가 계획과 해석을 맡으므로 별도 Codex·Claude CLI가 필요하지 않습니다. 단독 CLI는 `--agent codex`처럼 계획 모델을 선택합니다. 별도 모델의 추가 검토가 필요할 때만 `--review-with codex`를 사용합니다. 카탈로그·의미 검색은 모델 호출 전에 점검합니다. `goal` 자체는 활용신청을 자동 제출하지 않으므로, 필요한 API 접근권한은 [신청·조회 경로](#활용신청과-데이터-조회)로 먼저 준비합니다.
...
CLI와 MCP는 이 실행 플랫폼을 터미널과 AI 앱에서 사용하는 인터페이스이며, Agent Skill은 사용 절차를 안내합니다.

| 구성 | 역할 |
| --- | --- |
| Agent Skill | 요청에 맞는 사용 절차와 필요한 설정을 안내합니다. |
| MCP / CLI | AI 앱이나 터미널의 요청을 같은 실행 엔진으로 전달합니다. |
| `goal` | 목표에 필요한 자료를 찾고, 수집·계산·재탐색과 결과 검토를 진행합니다. |
...
`goal`의 단독 CLI 계획기는 Codex·Claude·Gemini를 지원합니다. MCP는 현재 연결된 AI를 사용합니다.
상세 행동 지침은 필요할 때 읽고, MCP는 변경된 상태만 전달합니다. 별도로 호출한 모델의 사용량은
`modelUsage`에 기록하며, 연결된 AI 자체의 토큰 사용량은 해당 앱에서 확인합니다.
