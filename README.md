...
적용된다. MCP에서는 같은 동작을 `advance_goal`로 host가 진행하며 중첩 모델을 호출하지 않는다.
...
안에서 추가 근거와 대안을 찾는다. 현재는 의미 승인 경로가 미완성이므로 목표 완료를 자동 선언하지
않으며 CLI도 성공 종료하지 않는다. 결과의 판정은 실제 실행 revision에 묶어 반환한다.
